...
usage() {
  cat <<'USAGE'
Usage:
  workflows/evaluate/run.sh --benchmark humaneval|mbpp_evalplus|livecodebench --model MODEL --split SPLIT --out-dir OUT_DIR [options]
...
case "${BENCHMARK}" in
  humaneval)
    ...
    SCORE_SCRIPT="workflows/evaluate/score_humaneval.py"
    DEFAULT_MAX_NEW_TOKENS=512
    ;;
  mbpp_evalplus)
    PROMPT_MODE="mbpp_evalplus_official"
    SCORE_SCRIPT="workflows/evaluate/score_mbpp.py"
    DEFAULT_MAX_NEW_TOKENS=512
    ;;
  livecodebench)
    PROMPT_MODE="livecodebench_official"
    SCORE_SCRIPT="workflows/evaluate/score_livecodebench.py"
...
GEN_ARGS=(
  python workflows/evaluate/generate.py
  --model "${MODEL}"
  --split "${SPLIT}"
