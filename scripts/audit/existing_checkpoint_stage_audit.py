...
FLAB_ROOT = ROOT / "results/raw/flabpruner_qwen25c15b_official_keep80_20260730_015031"
LLM_ROOT = ROOT / "results/raw/llmpruner_qwen25c15b_official_keep80_gpu_20260730_105340"
SLICE_ROOT = ROOT / "results/raw/slicegpt_qwen25c15b_official_keep80_gpu_20260731_011001"
...
def fixed_smoke_rows(path: Path) -> list[dict]:
    ...
    specs = [
        ("humaneval", ROOT / "data/splits/humaneval_half/eval.jsonl", 8, "humaneval_official"),
        ("mbpp_evalplus", ROOT / "data/splits/mbpp_evalplus_half/eval.jsonl", 8, "mbpp_evalplus_official"),
        ("livecodebench", ROOT / "data/splits/livecodebench_half/eval.jsonl", 4, "livecodebench_official"),
    ]
    rows=[]
...
def score_stage(stage_dir: Path, fixed_split_dir: Path):
    details={}
    commands=[
        [sys.executable, "scripts/eval/score_humaneval_official.py", "--split", str(fixed_split_dir/"humaneval.jsonl"), "--samples", str(stage_dir/"humaneval/samples.jsonl"), "--out-dir", str(stage_dir/"humaneval/score"), "--base-only"],
        [sys.executable, "scripts/eval/score_mbpp_evalplus_official.py", "--split", str(fixed_split_dir/"mbpp_evalplus.jsonl"), "--samples", str(stage_dir/"mbpp_evalplus/samples.jsonl"), "--out-dir", str(stage_dir/"mbpp_evalplus/score"), "--base-only"],
        [sys.executable, "scripts/eval/score_livecodebench_official.py", "--split", str(fixed_split_dir/"livecodebench.jsonl"), "--generations", str(stage_dir/"livecodebench/generations.jsonl"), "--out-dir", str(stage_dir/"livecodebench/score"), "--lcb-release", "release_v1", "--lcb-config", "release_latest", "--timeout", "6", "--num-process-evaluate", "2"],
    ]
    for cmd in commands:
...
def main() -> int:
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT/"results/raw/existing_checkpoint_stage_audit")
