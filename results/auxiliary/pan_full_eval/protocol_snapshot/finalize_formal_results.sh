...
cp "$WIN/workflows/audit/aggregate_pan_formal_results.py" workflows/audit/aggregate_pan_formal_results.py
tr -d '\r' < workflows/audit/aggregate_pan_formal_results.py > /tmp/a.py && mv /tmp/a.py workflows/audit/aggregate_pan_formal_results.py

.venv-common/bin/python workflows/audit/aggregate_pan_formal_results.py \
  --formal-root "$FORMAL_ROOT" \
  --output results/auxiliary/pan_full_eval/pan_formal_comparison.csv

# Sync key artifacts to Windows clone
mkdir -p "$WIN/results/tables" "$WIN/data/benchmarks/smoke/mbpp_evalplus" "$WIN/reports/stage1"
cp results/auxiliary/pan_full_eval/pan_formal_comparison.csv "$WIN/results/tables/"
cp -r data/benchmarks/smoke/mbpp_evalplus/. "$WIN/data/benchmarks/smoke/mbpp_evalplus/" 2>/dev/null || true
cp results/auxiliary/pan_full_eval/split_leakage_check_formal.json "$WIN/reports/stage1/" 2>/dev/null || true
...
csv_path = root/'results/auxiliary/pan_full_eval/pan_formal_comparison.csv'
