...
HE_GUIDE=data/benchmarks/auxiliary_full_eval/humaneval/guide.jsonl
HE_EVAL=data/benchmarks/auxiliary_full_eval/humaneval/eval.jsonl
MB_GUIDE=data/benchmarks/auxiliary_full_eval/mbpp/guide.jsonl
MB_EVAL=data/benchmarks/auxiliary_full_eval/mbpp/eval.jsonl
...
prune_one() {
  ...
  mkdir -p "$run_dir"
  echo "=== prune $tag ==="
  .venv-wanda/bin/python methods/wanda/qwen_prune.py \
    --method "$method" \
    --model "$QWEN" \
    --guide-file "$guide" \
